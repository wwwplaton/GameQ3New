...
class Source extends \GameQ3\Protocols {
	...
	protected function _process_details($packets) {
		...
		if ($buf->getLength()) {
			$d = $buf->readInt8();
			if (($d & 0x80) == 0x80) {
				$this->result->addGeneral('port', $buf->readInt16());
			}
	
			if (($d & 0x10) == 0x10) {
				$this->result->addGeneral('steam_id', ($buf->readInt32() << 32) | $buf->readInt32());
			}
	
			if (($d & 0x40) == 0x40) {
				$this->result->addGeneral('spectator_port', $buf->readInt16());
				$this->result->addGeneral('spectator', $buf->readString());
			}
	
			if (($d & 0x20) == 0x20) {
				$this->result->addGeneral('keywords', $buf->readString());
			}
	
			if (($d & 0x1) == 0x1) {
				$this->result->addGeneral('game_id', ($buf->readInt32() << 32) | $buf->readInt32());
			}
		}
		unset($buf);
