...
class Sockets {
	...
	*/
	...
	private function _closeSocketUDP($sctn) {
		...
		if (is_a($this->sockets_udp[$sctn], "Socket")) {
	...
	private function _closeSocketStream($sid) {
		...
		if (is_a($this->sockets_stream[$sid], "Socket")) {
	...
	private function _writeSocketUDP($sid, &$packets) {
		...
		// socket failed.
		if (!isset($this->sockets_udp[$sctn]) || !is_a($this->sockets_udp[$sctn], "Socket")) {
			// first fail
			if (isset($this->sockets_udp[$sctn]) && !is_a($this->sockets_udp[$sctn], "Socket")) {
	...
	private function _writeSocketStream($sid, &$packets, $retry = false) {
		// socket failed.
		if (!isset($this->sockets_stream[$sid]) || !is_a($this->sockets_stream[$sid], "Socket")) {
			// first fail
			if (isset($this->sockets_stream[$sid]) && !is_a($this->sockets_stream[$sid], "Socket")) {
				// failed to recreate socket
