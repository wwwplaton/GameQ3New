...
abstract class Protocols {
	...
	public function __construct($server_info, $log) {
		...
		if ($this->network) {
			...
			if (isset($server_info['ip'])) { // принимаем поле IP
				$this->connect_addr = $server_info['ip'];
				$this->ip = true;
			} else
			...
			if (isset($server_info['port'])) $query_port = $this->filterPort($server_info['port']); // port уже был
			
			if (isset($server_info['qport'])) { // qport уже был
				$this->query_addr = $server_info['qport'];
			} else
			...
			if (!$this->connect_addr && !$this->query_addr && !$this->ip) {
